...
app.get("/certificate_of_registration/:person_id", async (req, res) => {
  const { person_id } = req.params;

  try {
    const [results] = await pool.query(
      "SELECT * FROM certificate_of_registration WHERE person_id = ?",
      [person_id]
    );
    res.status(200).send(results);
  } catch (error) {
    res.status(500).send({ message: "Error fetching COR", error });
  }
});
...
app.post('/certificate_of_registration', (req, res) => {
  const query = `
    INSERT INTO certificate_of_registration (
      person_id, registration_no, academic_year_term, student_no, name, gender, age,
      ...
      computer_fee, total_assessment, less_financial_aid,
  ...
  pool.query(query, Object.values(req.body), (err, result) => {
    ...
    res.status(201).send({ message: 'Certificate of registration created', person_id: result.insertId });
  });
});


// Update Certificate of Registration Entry by person_id
app.put('/certificate_of_registration/:person_id', (req, res) => {
  const {
    registration_no,
    academic_year_term,
    student_no,
  ...
  pool.query(query, [
    ...
    if (err) return res.status(500).send({ message: 'Internal Server Error' });
    res.status(200).send({ message: 'Certificate of registration updated' });
  });
});

// DELETE certificate_of_registration by person_id
app.delete('/certificate_of_registration/:person_id', (req, res) => {
  const { person_id } = req.params;
  pool.query('DELETE FROM certificate_of_registration WHERE person_id = ?', [person_id], (err) => {
    if (err) return res.status(500).send(err);
